 ...
 */
#ifndef TRANSFORM_OBJ_RUGGEDNESS
#define TRANSFORM_OBJ_RUGGEDNESS
...
typedef struct {
  double offset;
} transform_obj_ruggedness_data_t;
 ...
 */
static void transform_obj_ruggedness_evaluate(IOHprofiler_problem_t *problem, const int *x, double *y) {
  transform_obj_ruggedness_data_t *data;
  size_t i;
  ...
  data = (transform_obj_ruggedness_data_t *) IOHprofiler_problem_transformed_get_data(problem);
 ...
 */
static IOHprofiler_problem_t *transform_obj_ruggedness(IOHprofiler_problem_t *inner_problem,const int *offset, double *y) {
  /*if(*offset==1){return inner_problem;}*/
  if(*offset>=0){return inner_problem;}
  IOHprofiler_problem_t *problem;
  transform_obj_ruggedness_data_t *data;
  size_t i;
  data = (transform_obj_ruggedness_data_t *) IOHprofiler_allocate_memory(sizeof(*data));

  problem = IOHprofiler_problem_transformed_allocate(inner_problem, data, NULL, "transform_obj_ruggedness1");
  problem->evaluate_function = transform_obj_ruggedness_evaluate;
  for (i = 0; i < problem->number_of_objectives; i++) {
