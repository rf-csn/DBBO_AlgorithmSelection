...
void readcfg(char * filename){
  ...
  layer_number = get_Dict_Int(dict,"suite","layer_number");
  layer_parameters=malloc(layer_number*50*sizeof(char));
  for(i = 0; i < layer_number; ++i){
    char buff[3],name[10];    
    sprintf(buff,"%d",i);
    strcpy(name, "layer");
    strcat(name,buff);
    layer_parameters[i] = get_Dict_String(dict,"suite",name);
  }}
